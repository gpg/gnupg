...
{
    hd->a = U64_C(0x0123456789abcdef);
    hd->b = U64_C(0xfedcba9876543210);
    hd->c = U64_C(0xf096a5b4c3b2e187);
    hd->nblocks = 0;
    hd->count = 0;
}
...
key_schedule( u64 *x )
{
    x[0] -= x[7] ^ U64_C(0xa5a5a5a5a5a5a5a5);
    x[1] ^= x[0];
    x[2] += x[1];
    x[3] -= x[2] ^ ((~x[1]) << 19 );
    x[4] ^= x[3];
    ...
    x[5] ^= x[4];
    x[6] += x[5];
    x[7] -= x[6] ^ U64_C(0x0123456789abcdef);
}
